...
`"RegisterTMR"`:: Triplicate only flip-flops and insert voters directly after their outputs. This has lower area and timing overhead than LogicTMR, protects register SEUs, and does not protect SETs in shared combinational logic.
...
| Bool
| `true`
| Validate voter inputs before insertion. Unconnected inputs are treated as errors and constant inputs generate warnings.
|===
...
[TIP]
====
Keep `tmr_voter_safe_mode = true` unless you have a very specific reason to disable it.
====

The equivalent Verilog attribute is `tmrx_tmr_voter_safe_mode`.

== Logic TMR Options

Configure in a `[<scope>.logic]` block such as `[global.logic]` or `[module.alu.logic]`.
The flip-flop detection options below apply to both `LogicTMR` and `RegisterTMR`.
...
| Bool
| `true`
| Insert voters after flip-flop outputs in LogicTMR. RegisterTMR always inserts these voters.
...
| Bool
| `false`
| When an FF clock-enable is inactive, load the voted state on the next active clock edge instead of holding the local lane state. LogicTMR requires `insert_voter_after_ff = true`; RegisterTMR always has the required voters.

| `ff_port_mappings`
| Table
| `{}`
| Port descriptions for PDK FF cells. Yosys built-in FF cells are recognized automatically.
...
With `correction_feedback = true`, TMRX replaces clock-enable hold behavior with a data mux:

* enable active: load the original data input;
* enable inactive: load the majority-voted FF output;
* the physical FF enable is held active so correction occurs on the next active clock edge.

This preserves normal updates while allowing one corrupted lane to rejoin the majority during an
otherwise idle cycle. FFs without a clock-enable are unchanged. The option is disabled by default.

Yosys built-in enabled FFs are handled automatically. PDK cells listed in `ff_cells` or
`additional_ff_cells` require a mapping:

[source,toml]
----
[global.logic]
correction_feedback = true
additional_ff_cells = ["MY_DFFE"]

[global.logic.ff_port_mappings.MY_DFFE]
clock_port = "CLK"
data_port = "D"
output_port = "Q"
enable_port = "EN_N"
...
`clock_port`, `data_port`, and `output_port` are required. `enable_port` is optional; omitting it
declares that the cell has no clock-enable and therefore needs no correction-feedback rewrite.
`enable_active_high` defaults to `true`.

[IMPORTANT]
====
Mapped PDK cells must give reset and asynchronous-load controls priority over clock enable.
Enable-over-synchronous-reset cells such as Yosys `$sdffce` are rejected; run `dffunmap` before
TMRX if such cells are present.
====
