  ...
  public:
    ...
    insert_duplicate_logic(RTLIL::Module *mod, std::vector<RTLIL::Wire *> wires,
        ...
        for (auto w : wires) {
            ...
            if ((cfg->preserve_module_ports && w->port_input) ||
                (w->name == cfg->clock_port_name && !cfg->expand_clock) ||
                (w->name == cfg->reset_port_name && !cfg->expand_reset)) {
                wire_map[w] = w;
                continue;
            }
    ...
    insert_output_voters(RTLIL::Module *mod,
        ...
        for (auto outputs : out_map) {
            ...
            mod->rename(res_wires.first, mod->uniquify(outputs.first->name.str().substr(
                                             0, outputs.first->name.str().size() -
                                                    (cfg->logic_path_1_suffix.size()))));
        }
    ...
    std::vector<RTLIL::Wire *>
    insert_voter_after_ff(RTLIL::Module *mod, dict<Cell *, std::pair<Cell *, Cell *>> ff_map) {
        std::vector<RTLIL::Wire *> error_signals;

        for (auto flip_flops : ff_map) {

            std::vector<RTLIL::IdString> output_ports =
                get_output_port_name(flip_flops.first, mod->design);
            ...
            for (auto port : output_ports) {
                ...
                for (auto ff :
                     {flip_flops.first, flip_flops.second.first, flip_flops.second.second}) {
    ...
    std::vector<RTLIL::Wire *> connect_submodules_mod_ports(
        RTLIL::Module *mod, RTLIL::Cell *cell, const Config *cell_cfg,
        dict<RTLIL::SigSpec, std::pair<RTLIL::SigSpec, RTLIL::SigSpec>> wire_map) {
    ...
    void logic_tmr_expansion(RTLIL::Module *mod, const ConfigManager *cfg_mgr) {
        ...
        auto [wiremap_b, outputmap_b, flipflopmap_b] =
            insert_duplicate_logic(mod, original_wires, original_cells, original_connections,
                                   cfg->logic_path_2_suffix, cfg);
        auto [wiremap_c, outputmap_c, flipflopmap_c] =
            insert_duplicate_logic(mod, original_wires, original_cells, original_connections,
                                   cfg->logic_path_3_suffix, cfg);
    ...
    void full_module_tmr_expansion(RTLIL::Module *mod, const Config *cfg) {
        ...
        if (cfg->tmr_mode_full_module_insert_voter_before_modules && !cfg->preserve_module_ports) {
            for (auto wm : wire_map) {
                ...
                for (size_t i = 0; i < 3; i++) {
                    auto [v_out, err] =
                        insert_voter(wrapper, {wm.second.at(0), wm.second.at(1), wm.second.at(2)},
                                     wrapper->design);
