  ...
  public:
    ...
    std::tuple<dict<RTLIL::SigSpec, RTLIL::SigSpec> , dict<RTLIL::Wire *, RTLIL::Wire *>, dict<RTLIL::Cell*,RTLIL::Cell*>> insert_duplicate_logic(RTLIL::Module *mod, std::vector<RTLIL::Wire*> wires, std::vector<RTLIL::Cell*> cells,std::vector<RTLIL::SigSig> connections, std::string suffix, const Config *cfg){
        ...
        for(auto w : wires){
            ...
            if((cfg->preserve_module_ports && w->port_input) || (is_clk_wire(w,cfg) && !cfg->expand_clock) || (is_rst_wire(w, cfg) && !cfg->expand_reset)){
                wire_map[w] = w;
                continue;
            }
    ...
    void rename_wires_and_cells(RTLIL::Module *mod, std::vector<RTLIL::Wire*> wires, std::vector<RTLIL::Cell*> cells, std::string suffix, const Config *cfg){
        ...
        for (auto w : wires) {
          if(is_tmr_error_out_wire(w) || (cfg->preserve_module_ports && w->port_input) || (is_clk_wire(w,cfg) && !cfg->expand_clock) || (is_rst_wire(w, cfg) && !cfg->expand_reset)){
            continue;
          }
    ...
    void full_module_tmr_expansion(RTLIL::Module *mod, const Config *cfg){
        ...
        for(size_t i = 0; i < 3; i++){
            for (auto w : mod->wires()) {
                ...
                bool ignore_wire = (is_clk_wire(w, cfg) && !cfg->expand_clock) || (is_rst_wire(w, cfg) && !cfg->expand_reset) || is_tmr_error_out_wire(w);
		...
		if(cfg->tmr_mode_full_module_insert_voter_before_modules && !cfg->preserve_module_ports){
            for(auto wm : wire_map){
                if( !wm.first->port_input ||  (is_clk_wire(wm.first, cfg) && !cfg->expand_clock) || (is_rst_wire(wm.first, cfg) && !cfg->expand_reset)){
                        continue;
                }
		...
		if(cfg->tmr_mode_full_module_insert_voter_after_modules && !cfg->preserve_module_ports){
            for(auto wm : wire_map){
                if( !wm.first->port_output || is_tmr_error_out_wire(wm.first) || (is_clk_wire(wm.first, cfg) && !cfg->expand_clock) || (is_rst_wire(wm.first, cfg) && !cfg->expand_reset)){
                        continue;
                }
